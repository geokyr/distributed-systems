...
def create_transaction():
    ...
    if (receiver_public_key and receiver_id != node.id):
        ...
        if creation["success"]:
            return jsonify({'message': 'The transaction was successful.', 'balance': node.wallet.wallet_balance(), "mining_time": creation["mining_time"]}), 200
        else:
            return jsonify({'message': 'Not enough NBCs.', 'balance': node.wallet.wallet_balance(), "mining_time": creation["mining_time"]}), 400
    else:
        return jsonify({'message': 'Transaction failed. Wrong receiver id.', "mining_time": creation["mining_time"]}), 400
...
def get_my_transactions():
    '''Endpoint that returns all the transactions of a node (as a sender of receiver).
        ...
        Returns:
            a formatted list of transactions in pickle format.
    '''
    return pickle.dumps([tr.convert_to_list() for tr in node.wallet.transactions])
